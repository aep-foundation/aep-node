...
const routes = {
  enroll: createNextAepRoute(service, "enroll"),
  grant: createNextAepRoute(service, "grant"),
  inspect: createNextAepRoute(service, "inspect"),
  revoke: createNextAepRoute(service, "revoke"),
  status: createNextAepRoute(service, "status")
};
...
async function handleNodeRequest(
  ...
  const webResponse = await routeResponse(webRequest);
...
async function routeResponse(request: Request): Promise<Response> {
  ...
  if (request.method === "POST" && url.pathname === "/aep/enroll") {
    return requireHandler(routes.enroll.POST)(request);
  }

  if (request.method === "GET" && url.pathname === "/aep/status") {
    return requireHandler(routes.status.GET)(request);
  }

  if (request.method === "POST" && url.pathname === "/aep/grant") {
    return requireHandler(routes.grant.POST)(request);
  }

  if (request.method === "POST" && url.pathname === "/aep/revoke") {
    return requireHandler(routes.revoke.POST)(request);
  }

  if (request.method === "GET" && url.pathname === "/api/resource") {
    const denied = await deniedProtectedResponse(request);

    return denied ?? jsonResponse(200, resourceBody());
  }
...
function requireHandler<THandler>(handler: THandler | undefined): Exclude<THandler, undefined> {
  if (handler === undefined) {
    throw new Error("Expected route handler.");
  }

  return handler as Exclude<THandler, undefined>;
}
