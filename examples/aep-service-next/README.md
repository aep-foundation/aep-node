# Example - AEP Service on Next route handlers

A Next route-handler style Service that mounts AEP routes and protects application routes with AEP client assertion JWTs. The example runs those handlers behind a small Node HTTP server so it can be launched like the other examples.

It advertises no grant types, so Agent examples use the JWT fallback mode.
